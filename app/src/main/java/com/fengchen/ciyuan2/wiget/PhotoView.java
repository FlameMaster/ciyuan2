...
//        setBackgroundColor(Color.WHITE);
    ...
    private void initData() {
        ...
        //图片宽度为屏幕宽度除缩放倍数
        mImageWidth = getMeasuredWidth() / values[Matrix.MSCALE_X];
        mImageHeight = (getMeasuredHeight()- values[Matrix.MTRANS_Y] * 2) / values[Matrix.MSCALE_Y];
        mScale = values[Matrix.MSCALE_X];
        //保证精确测量
        if (getWidth()==0||getWidth()!=getMeasuredWidth()){
        ...
        Log.e("PhotoView",new StringBuffer("-->\n")
                .append("mScale[").append(mScale).append("]\n")
                .append("getWidth()[").append(getWidth()).append("]\n")
                .append("getMeasuredWidth()[").append(getMeasuredWidth()).append("]\n")
                .append("mImageWidth[").append(mImageWidth).append("]\n")
                .append("mImageHeight[").append(mImageHeight).append("]\n")
        .toString());
    }
    ...
    public class MatrixTouchListener implements OnTouchListener {
        ...
        float mMaxScale = 6;
        /*双击时的缩放级别：多重缩放*/
        float[] mDobleClickScale = new float[]{2, 4};
        private int mMode = 0;//
        /*缩放开始时的手指间距*/
...
///////////////////////////////////用于记录开始时候的坐标位置///////////////////////////
        ...
        boolean mLeftDragable, mRightDragable;
        /*是否第一次移动*/
        ...
        /*判断缩放级别是否是改变过：true表示非初始值, false表示初始值*/
...
//            float scale = isZoomChanged() ? 1 : mDobleClickScale[0];
            ...
            B:
            if (scale != mScale) {
                for (int i = 0; i < mDobleClickScale.length; i++) {
                    if (i == mDobleClickScale.length - 1) {
                        scale = 1;
                        break B;
                    } else if (scale == mDobleClickScale[i]*mScale) {
                        scale = mDobleClickScale[i + 1];
                        break B;
                    }
                }
            } else scale = mDobleClickScale[0];

//            FCUtils.showToast("倍数："+scale);
